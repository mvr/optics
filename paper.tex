...
\end{center}
reading left to right, so the left half of the diagram represents $l$ and the right half $r$. The relation imposed by the coend takes the following form:
\begin{center}
\input{diagrams/coend-relation-left.tikz} \quad $=$ \quad \input{diagrams/coend-relation-right.tikz}
\end{center}
...
&= \langle l, r \rangle
\end{align*}
Where in both cases we have used the naturality of $\lambda$ and coend relations to move $\lambda^{-1}_M$ to the right-hand side of the coend. Graphically:
\begin{center}
\todo{todo}
\end{center}
...
\begin{proposition}
For any $\C$, there is a functor $\iota : \C \times \C^\op \to \Optic_\C$, which is given on objects by $\iota(S, S') = (S, S')$ and for a morphism $(f, g) : (S, S') \to (A, A')$ by $\iota(f, g) = \langle \lambda_A^{-1} f, g \lambda_{A'} \rangle$.
\end{proposition}
\begin{proof}
...
\begin{proposition}
A strong monoidal functor $F : \C \to \D$ induces a functor $\Optic(F) : \Optic_\C \to \Optic_\D$
\end{proposition}
\begin{proof}
On objects this is simply $\Optic(F)(S, S') = (FS, FS')$. The action on morphisms $\Optic_\C((S, S'), (A, A'))$ is induced by
\begin{align*}
&\C(S, M \otimes A) \times \C(M \otimes A', S) \\
\to &\D(FS, F(M \otimes A)) \times \D(F(M \otimes A'), FS) \\
\to &\D(FS, FM \otimes FA) \times \D(FM \otimes FA', FS) \\
\to &\int^{N \in \D} \D(FS, N \otimes FA) \times \D(N \otimes FA', FS).
\end{align*}
where we have used the strong monoidal isomorphism in both directions.
\todo{use equations instead}
\todo{argue for functoriality}
\end{proof}
...
As noted in \cite{CoherenceForLenses}, \todo{the category of optics admits its own string diagram language}
...
\begin{itemize}
\item A wide symmetric monoidal subcategory $\C_d$ of dualisable morphisms, with an involutive strong symmetric monoidal functor $(-)^* : \C_d^\op \to \C_d$
...
\end{itemize}
\end{definition}
\todo{The involution bit seems to be almost the same as asking for a symmetric monoidal dagger-category, that is stricter than this though}

Unpacking the definition, $\varepsilon$ being a monoidal extranatural transformation amounts to the following diagrams in $\C$ commuting:
\[
\begin{tikzcd}
...
\end{tikzcd} \hspace{1cm}
\begin{tikzcd}
X \otimes Y \otimes X^* \otimes Y^* \ar[r, "X \otimes s \otimes X^*"]  \ar[dr, "\varepsilon_{X \otimes Y}", swap] & X \otimes X^* \otimes Y \otimes Y^* \ar[d, "\varepsilon_X \otimes \varepsilon_Y"] \\
& I
\end{tikzcd}
\] 
where $f : X \to Y$ is in $\C_d$. \todo{Is that last one actually something extra? It seems like the duality should also reverse the order of tensor, or something} 

\begin{proposition}
$\Optic_\C$ forms a teleological category, with the following data:
\begin{itemize}
...
\item The counit $\varepsilon_{(S, S')}$  is given by the image of the symmetry $s_{S \otimes S'}$ under the isomorphism of Proposition \ref{prop-costates}.
\end{itemize}
\end{proposition}
\begin{proof}
That morphisms of the form $\iota(f, g)$ constitute a category and that $\iota$ is a symmetric monoidal involution is clear. 
...
\todo{They stress about the empty set, does this matter here?}

As conjectured by Jules Hedges, the structure of a teleological category is precisely what is added by forming the $\Optic$ category.
\begin{theorem}
$\Optic_\C$ is the free teleological category on a symmetric monoidal category $\C$.
\end{theorem}
\begin{proof}
\todo{todo}
\end{proof}

\todo{Consequences: Inclusion $\C \to \Optic_\C$ is faithful/monoidal?}

\subsection{Optics for a Monoidal Action}

To capture the \texttt{Iso}s, \texttt{Traversal}s and \texttt{Setter}s of the Haskell \texttt{lens} library, we generalise to the case of a monoidal action of one category on another.
...
\begin{remark}
In this section we will focus only on preoptics of the form $p : (S,S) \hto (A, A)$. We therefore abbreviate $\Optic_\M((S, S), (A, A))$ as $\Optic_\M(S, A)$ and $p : (S, S) \hto (A, A)$ as $p : S \hto A$.
\end{remark}
...
\begin{proposition}
There is an subcategory $\Lawful_\C$ of $\Optic_\C$ given by objects of the form $(X, X)$ and lawful optics between them.
\end{proposition}
\begin{proof}
The identity preoptic $\iota(\id_X, \id_X)$ is clearly an optic. 
...
\begin{definition}
Suppose $\C$ has finite products, and let $\C$ act on itself via the bifunctor $\times : \C \times \C \to \C$. The \emph{category of lenses} $\Lens$ is the category of optics with respect to this action: $\Lens = \Lawful_\times$.
\end{definition}
...
\begin{align*}
\Optic_\times(S, A) &= \int^{M \in \M} \C(S, M \times A) \times \C(M \times A, S) \\
...
\begin{align*}
\Optic_\sqcup(S, A) &= \int^{M \in \M} \C(S, M \sqcup A) \times \C(M \sqcup A, S) \\
