...
\begin{proof}
  ...
  To check extranaturality of $\varepsilon$, suppose we have a dualisable optic $\iota(f, g) : (S, S') \hto (T, T')$, so $f : S \to T$ and $g : T' \to S'$. Luckily here all the switching in the definitions scancels out! Extranaturality is witnessed by the equality of the string diagrams:
  \begin{center}
...
\begin{remark}
This characterisation of setters is maybe a little odd, in that we have ended up with a function of $\Set$s, rather than a description internal to $\C$. If we modify our definition of $\Setter$ slightly, we can get such an internal characterisation.

Suppose $\C$ is cartesian closed and let $\Strong_\C$ be the category of \emph{strong functors}\cite{StrongFunctors} on $\C$. Then, again, $\Strong_\C$ acts on $\C$ by evaluation. We leave it to the reader to verify there is a natural isomorphism \[\C(A, FB) \cong \Strong_\C(A \times \homC(B, -), F),\] which we can use to describe optics for this action as elements of $\C(\homC(A, A'), \homC(S,S'))$.
\end{remark}
...
Finally, this functor $a$ is monoidal, with the structure maps given by the associativity and unit maps of $\C$. \todo{this uses that the strength commutes with unitors and associativity}
\end{proof}
