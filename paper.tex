...
\end{itemize}
We will call such lenses \emph{lawful}. (The names of the laws make more sense when the equations are written with composition in diagrammatic order!) Lenses form a category with a subcategory of lawful lenses.
...
Our definition of optic has as domain and codomain \emph{pairs} of objects of $\C$. This generality is important for the applications to game theory; for ordinary lenses we restrict our attention to pairs with the same object in both entries.
...
\begin{remark}
  ...
  \end{center}
  We will therefore omit the vertical cut point between $l$ and $r$ in most subsequent diagrams; any choice yields a representative of same optic.
\end{remark}
...
Using the isomorphism in both directions, we leave it to the reader to verify that composition in $\Optic_\times$ corresponds to ordinary composition of concrete lenses. (Of course, there is only one possible way such a collection of morphisms could be sensibly composed!)
\end{remark}
...
\end{align*}
The category $\mathsf{Aff}$ acts on $\C$ by $(P, Q) \act A = P \sqcup (Q \times A)$, in fact, $\mathsf{Aff}$ is cooked up exactly as the closure of the actions $- \times A$ and $- \sqcup A$ under composition. A concrete affine traversal is an element of \[\C(S, S' \sqcup (\homC(A', S') \times A)).\]
...
We can generalise the definition of $\Optic$ so that the two halves lie in different categories, allowing us to capture a few more optic variants found in the wild. Suppose $\C_1$ and $\C_2$ are categories that are each acted on by a common monoidal category $\M$.

\begin{definition}
  Given two objects of $\C_1 \times \C_2^\op$, say $(S, S')$ and $(A, A')$, an \emph{optic} $p : (S, S') \hto (A, A')$ is an element of the set
  \begin{align*}
    \Optic_\otimes((S, S'), (A, A')) := \int^{M \in \M} \C_1(S, M \act A) \times \C_2(M \act A', S')
  \end{align*}
\end{definition}

Several results from Section~\ref{sec:optics} continue to hold:
\begin{proposition}\label{prop:mixed-optic-is-cat}
  The above data form a category $\Optic_{\M, \C_1, \C_2}$. \todo{notation?} \qed
\end{proposition}
\begin{proposition}
  There is a functor $\iota : \C_1 \times \C_2^\op \to \Optic_{\M, \C_1, \C_2}$, which on objects is given by $\iota(S, S') = (S, S')$ and on a morphism $(f, g) : (S, S') \to (A, A')$ by \[\iota(f, g) = \rep{(\lambda^{\C_1}_A)^{-1} f}{g \lambda^{\C_2}_{A'}},\] where $\lambda^{\C_1}_A : A \to IA$ in $\C_1$ and $\lambda^{\C_2}_{A'} : A' \to IA'$ in $\C_2$. \qed
\end{proposition}
\begin{proposition}\label{prop:mixed-change-of-action}
  Given two morphisms of actions $F_1 : (\M, \C_1) \to (\N, \D_1)$ and $F_2 : (\M, \C_2) \to (\N, \D_2)$, there is an induced functor $\Optic(F_1, F_2) : \Optic_{\M, \C_1, \C_2} \to \Optic_{\N, \D_1, \D_2}$. \qed
\end{proposition}

The notion of lawfulness used earlier breaks down in this setting, as the definition of $\Twoptic(S, A)$ only makes sense when $\C_1 = \C_2$. The following definition gives a reasonable approximation.

\begin{definition}
If $F_1 : (\M, \C_1) \to (\N, \D)$ and $F_2 : (\M, \C_2) \to (\N, \D)$, a mixed optic $p : (S, S) \hto (A, A)$ in $\Optic_{\M, \C_1, \C_2}$ is \emph{lawful relative to $(F_1, F_2)$} if its image under $\Optic(F_1, F_2)$ is lawful.
\end{definition}

\subsection{Examples}

\subsubsection{Degenerate Optics}
...
&\cong \C(S, A)
\end{align*}
This works for $\Review := \Optic_{\sqcup, \C_\codisc, \C}$ in the same way. There are induced functors $\Optic(\id_\C, i) : \Lens \to \Getter$ and $\Optic(i, \id_\C) : \Prism \to \Review$.
...
\subsubsection{Indexed Optics}
As well as giving a principled home to degenerate optics, the notion of mixed optics also allows us to describe \emph{indexed optics} categorically. The most interesting example of an indexed optic variant is the \emph{indexed traversal}, which, for example, allows access to the indices of the entries of a list as it is being traversed.

We first need to define indexed traversable functors. Unfortunately this has many moving parts!

\begin{proposition}
The assignment $F \act X \mapsto F \Set(I, X)$ is a lax monoidal action of $\App$ on $\Set$, called the \emph{$I$-indexed action of $\App$ on $\Set$}.
\end{proposition}
...
\begin{proposition}
$\IdxTraversable$ acts on $\C \times \C$ by $(T, I) \act (A, R) \mapsto (TA, \homC(I, R))$ and on $\C$ simply by $(T, I) \act A \mapsto TA$.
\end{proposition}
