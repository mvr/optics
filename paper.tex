...
\end{align*}
%Let $\conc_\times((S, S'), (A, A'))$ denote this set of concrete lenses. 
...
\begin{theorem}\label{thm:optic-functor}
  The $\Optic_\otimes$ construction extends to a functor \[\Optic : \SymmMonCat \to \SymmMonCat,\] where $\SymmMonCat$ denotes the (1-)category of (small) symmetric monoidal categories and symmetric monoidal functors.
\end{theorem}
...
\begin{proposition}
  If $\C$ is a strict symmetric monoidal category then $\Optic_\otimes$ is strict, and $\iota : \C \times \C^\op \to \Optic_\otimes$ is a strict monoidal functor. For $F$ a strict monoidal functor, the induced $\Optic(F)$ is also strict.
\end{proposition}
\begin{proof}
...
It will be convenient to equip $\Optic_\otimes$ with a slightly different symmetric monoidal structure.
...
\begin{proposition}
  $(\Optic_\otimes, \switched, (I, I))$ is a symmetric monoidal category. 
  %that is monoidally equivalent to $\Optic_\otimes$ with the unswitched tensor.
\end{proposition}
\begin{proof}
...
We have previously noted that $\Optic_\otimes$ is strict monoidal if $\C$ is, and that in that case the duality is strict. There are functors
\begin{align*}
...
\begin{proposition}
  Suppose $(\C, \otimes, I)$ is a symmetric strict monoidal category and $(\T, \teletimes, I, {(-)}^*, \varepsilon)$ is a strict teleological category. Given a strict symmetric monoidal functor $F : \C \to \T_d$, there exists a unique strict teleological functor $K : \Optic_\otimes \to \T$ with the property $Kj = F$.
\end{proposition}
\begin{proof}
  We construct $K$ as follows. Note that any object $(S, S')$ in $\Optic_\otimes$ can be written uniquely as $j(S) \switched {j(S')}^*$, so we are forced to define $K(S, S') = FS \teletimes {(FS')}^*$. Suppose $\rep{l}{r} : (S, S') \hto (A, A')$ is an optic. By the previous Proposition,
  \begin{align*}
    \rep{l}{r} = ((A, I) \switched \varepsilon_{(M, I)} \switched (I, A'))(j(s_{M,A}l) \switched {j(rs_{A', M})}^*)
  \end{align*}
...
This is automatically natural in $\T$. Naturality in $\C$ follows by Lemma~\ref{lem:iota-commute-with-opticf}. 
\end{proof}
...
Of course, this notion of effectful lens may not be useful! It is hard to get intuition for the meaning of the laws, but they seem to suffer from the same deficiencies that other attempts do: they are too strong. The $\fget\fput$ law here appears easier to satisfy than the $\mathsf{MGetPut_0}$ law of~\cite{ReflectionsOnMonadicLenses}, as $\mput$ is given access to the original state. However, our $\fput\fget$ law seems very difficult to satisfy: no matter what auxiliary state is provided, $\fput$ting then $\fget$ting must leave the state unchanged.
...
Suppose $\C$ is cartesian closed. We define two actions of $\C \times \C$, the first on $\C$ and the second on $\C \times \C^\op$:
\begin{align*}
M \actL A  &:= M \times A\\
...
\begin{align*}
  ...
  &\cong \C(S, A) \times \C(S \times A', S') \times (S \times J', I')
\end{align*}

\todo{talk about polymorphism to manage the indices}

\begin{remark}
There appears to be some flexibility in what actions are used here. To have the indices behave covariantly rather than contravariantly, we could have $\C$ act on $\C$ and $\C \times \C$ as follows:
\begin{align*}
M \actL A &:= M \times A \\
...
\end{align*}
We chose the previous, more contorted actions so that indexed lenses are more clearly related to indexed traversals, which we define presently.
%We calculate:
%\begin{align*}
%  &\Optic_{\actL, \actR}((S, (S', J')), (A, (A', I'))) \\
