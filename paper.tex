...
\item The counit $\varepsilon_{(S, S')}$  is given by the image of the symmetry $s_{S \otimes S'}$ under the isomorphism of Proposition \ref{prop-costates}. \todo{It is a little annoying that a twist is necessary, but it has to go somewhere. The other option is in the definition of $\otimes$ but that seems worse.}
\end{itemize}
\end{proposition}
\begin{proof}
That morphisms of the form $\iota(f, g)$ constitute a category and that $\iota$ is a symmetric monoidal involution is clear. \todo{actually check monoidal part}
...
As remarked in the introduction \todo{(todo: remark in the introduction)}, the optics for this action are exactly pairs of $\fget$ and $\fput$ functions.
\begin{align*}
...
\todo{Show that for the category of nonempty sets things .}
\end{remark}
...
This has been written out many times before in the case when $lr$ is the identity \cite{IsomorphismLensesPost}, we note here that the weaker condition is sufficient.
