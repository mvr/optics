...
The general pattern is, once we choose a particular monoidal action $\M \to [\C, \C]$, we can find an isomorphism between the set of optics $(S, S') \hto (A, A')$ and a set that is easier to describe. The goal is then to find necessary and sufficient conditions on this second set for the corresponding element of the coend to be a lawful optic.
...
&= \int^{M \in \C} \C(S, (M \sqcup 1) \times A) \times \C((M \times A') \sqcup A', S') \\
&= \int^{M \in \C} \C(S, (M \sqcup 1) \times A) \times \C(M \times A', S') \times \C(A', S') \\
&= \int^{M \in \C} \C(S, (M \sqcup 1) \times A) \times \C(M, \homC(A', S')) \times \C(A', S') \\
&= \C(S, (\homC(A', S') \sqcup 1) \times A) \times \C(A', S') \\
&= \C(S, \homC(A', S') \sqcup 1) \times \C(S, A) \times \C(A', S') \\
\end{align*}

\todo{Equivalently(?), let $\M$ be the category of pointed sets}
...
\begin{remark}
For this argument to work, it is important that the morphisms in $\mathsf{Aff}$ are just those in $\C \times \C$, even though there are likely to be more natural transformations between the functors $P \sqcup (Q \times -)$ and $P' \sqcup (Q' \times -)$.
\end{remark}
...
\begin{definition}
There is a category $\Tamb(\M)$ of Tambara modules and natural transformations that respect the extra structure, in the sense that for any $l : P \to Q$, the diagram
\[
\begin{tikzcd}
...
This category is monoidal with respect to $\odot$ as given above, and the monoidal unit is given by $I$. There is an evident forgetful functor $U : \Tamb(\M) \to \Prof(\C, \C)$ that is strong monoidal. This forgetful functor has both a left and right adjoint; particularly important for us is the left adjoint:
...
\todo{The converse is true in specific situations:}
...
\end{theorem}
\begin{proof}
The natural transformation $t$ is characterised by its component at $\Pastro_\M E_A$, so I expect the result to follow from the lax monoidalness of $t$ with $P = Q = \Pastro_\M E_A$. 

The triangle diagram gives immediately that $rl = \id_S$. The square diagram says that
\begin{align*}
...
\end{align*}
And the goal is to somehow prove that $\langle lr \rangle = \psi A$ for some $\psi : M \to M$. Presumably a proof would work by induction on the chain of coend relations, but I don't see how to make progress.
\end{proof}
...
Getters $(S, S') \hto (A, A')$ are supposed to correspond to morphisms $S \to A$. These cannot fit nicely into the coend formalism, as there is always the function $\mathsf{outside} : \Optic_\M((S, S), (A, A')) \to \C(S, S')$.
