...
\newcommand{\fget}{\mathrm{\underline{get}}}
\newcommand{\fput}{\mathrm{\underline{put}}}
\newcommand{\fmodify}{\underline{\mathrm{modify}}}
...
\todo{Might have to adjust the entire thing because the action of $\M$ on $\C$ is not strictly monoidal. On the other hand, through the entire Pastro/Street paper pretends associativity/etc is strict, so maybe who cares? What we really want is just a (nonstrict) monoidal functor $\M \to [\C, \C]$ from some $\M$.}
...
\begin{proposition}
Let $\C$ be a category and $\M$ a subcategory of $[\C, \C]$ closed under composition. There is a category $\PreOptic_\M$ that has the same objects as $\C \times \C$, with homsets given by
\begin{align*}
\PreOptic_\M((S, S'), (A, A')) = \int^{M \in \M} \C(S, M A) \times \C(M A', S')
\end{align*}
\end{proposition}
If $f : S \to M A$ and $g : M A' \to S'$, we write $\langle l, r \rangle$ for their image in $\PreOptic_\M((S, S'), (A, A'))$. We will refer to $M$ as the \emph{complement} for the pair $(l, r)$.
\begin{proof}
...
\end{align*}
In other words, let $\langle l, r \rangle \in \PreOptic_\M((R, R'), (S, S'))$ and $\langle l', r' \rangle \in \PreOptic_\M((S, S'), (A, A'))$ with $M$ the complement for $(f, g)$. The composite is then: $\langle l', r' \rangle \circ \langle l, r \rangle = \langle (M l')l, r(Mr') \rangle$
\end{proof}
...
If $p \in \PreOptic_\M(S, A)$ is an optic, then there exists a representative $p = \langle l, r \rangle$ such that $r \circ l = \id_{MA}$ on the nose for some $M$.
\end{proposition}
\begin{proof}
...
In fact we will prove the more general statement that for $p \in \PreOptic_\M(S, A)$, if $outside(p) = \id_S$ and $inside(p) = \langle t \rangle$ where $t : MA \to MA$ is idempotent, then we can find a representative $\langle l, r \rangle$ with $l \circ r = t$ on the nose.
...
\end{proposition}
\begin{proof}
The identity preoptic $\langle \id_X, \id_X \rangle$ is clearly an optic. Suppose we have two optics $\langle l, r \rangle \in \Optic_\M(R, S)$ and $\langle l', r' \rangle \in \Optic_\M(S, A)$, where $\langle l, r \rangle$ has complement $M$. Then the composite $\langle (Ml')l, r (Mr')  \rangle$ is certainly also an optic:
\begin{align*}
(Ml')l r (Mr') &= (Ml')(Mr') = M(l'r') = \id
\intertext{and}
r (Mr')(Ml')l &= r M(r'l') l = rl = \id
\end{align*}
\end{proof}

\subsection{Examples}

\todo{For lenses, not actually the full subcategory of $[\C, \C]$, just morphisms of the form $f \times \id$. This is a disadvantage of doing it as functors and not as an action.}

\subsection{Equivalence of Optics}

One issue with the definition of optic given in the last section is that it could be difficult to determine when two optics $\langle l, r \rangle, \langle l', r' \rangle \in \Optic_\M(S, A)$ are equal. For some choices of $\M$, however, we can reduce a zigzag of relations to a single one.
...
\begin{proposition}
Suppose all epimorphisms split in $\C$, and that $- \times A : \C \to \C$ reflects epis. Then two lenses $p = \langle l, r \rangle$ and $q = \langle l', r' \rangle \in \Optic_\times(S, A)$ with complements $M$ and $N$ respectively are equal iff there exists an morphism $\phi : M \to N$ such that $(\phi \times A)l = l'$ and $r = r' (\phi \times A)$.
\end{proposition}
\begin{proof}
...
\[
\begin{tikzcd}
M \times A \ar[r, "{(f, \id_A)}", dashed, bend left = 50] \ar[d, "r", swap] & S \times A \ar[d, "\fput"] \ar[r, "{(\pi_1 l', \id_A)}"] \ar[l, "{(\pi_1 l, \id_A)}", swap] & N \times A \ar[d, "r'"] \\
...
\todo{recall definitions} We use $I$ to refer to the identity profunctor $\C(-,{=})$, and $\odot$ for horizontal profunctor composition.
...
\end{corollary}
\begin{proof}
This follows by abstract nonsense as a left adjoint of a strong monoidal functor. \todo{reference?}
\end{proof}
...
\end{align*}
by the co-Yoneda lemma twice followed by the adjunction $\Pastro_\M \dashv U$.
\end{proof}
...
\begin{corollary}
A profunctor preoptic is completely determined by its component at $\Pastro_\M E_A$, and furthermore, this component is determine by its value on $\langle \id_A, \id_A, \id_A, \id_A \rangle \in (U \Pastro_\M E_A)(A, A)$.
\end{corollary}
\begin{proof}
...
\begin{proposition}
Suppose $p \in \Optic_\times(S,A)$ is a pre-lens such that the associated natural transformation $p : (U-)(A,A) \to (U-)(S,S)$ is lax monoidal. Then $p$ is a lens.
\end{proposition}
\begin{proof}
...
\todo{These maybe involve profunctors from $\D \times \C$ to $\C$ where the composition does something funny with $\D$.}

%\section{Merely Well-Behaved Optics}
\section{The Challenge of Effectful Optics}
%\section{Symmetric Optics}
