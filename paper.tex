...
\newcommand{\rep}[2]{{\ensuremath \langle #1 \mid #2 \rangle}}
...
If $l : S \to M \otimes A$ and $r : M \otimes A' \to S'$, we write $\rep{l}{r} : (S, S') \hto (A, A')$ for the image of this pair in $\Optic_\C((S, S'), (A, A'))$, and say that $M$ is the complement for the representative. Optics will always be written with a crossed arrow $\hto$, to distinguish them from morphisms of $\C$.
...
\begin{proof}
  ...
  \begin{align*}
    F\rep{ l}{ r} := \rep{\phi^{-1}_{M,A} (Fl)}{(Fr) \phi_{M,A'}}
  \end{align*}
...
\end{theorem}
\begin{proof}
  On objects, define $K(S, S') = FS \otimes (FS')^*$. Suppose $\rep{ l}{ r} : (S, S') \hto (A, A')$ is an optic. By the previous Proposition,
  \begin{align*}
...
\todo{In this section I am using semicolon to mean composition in diagrammatic order. Should I also switch the comma in $\rep{ l}{ r}$ to be a $\mid$?}

For a fixed $M \in \M$, there is a map $\C(S, M A) \times \C(M A, S) \to \C(S, S)$ given by composition. This induces a map $\outside : \Optic_\M(S, A) \to \C(S, S)$, given equationally by $\outside(\rep{ l}{ r}) = l ; r$
...
\begin{align*}
  ...
  \twice(\rep{ l}{ r} \rangle) &= \repthree{l}{r;l}{r}
\end{align*}
...
\begin{proof}
  ...
  \[ \repthree{ l;(Ml')}{ (Mr'); r;l;(Ml')}{(Mr') ; r } = \rep{ l;(Ml')}{ \id_{MNA}}{ (Mr') ; r }. \]
  ...
  So consider the generating relation $\repthree{ l;\phi_S}{ c}{r} = \rep{ l}{ \phi_S ; c}{r}$ in \[\int^{M_1, M_2 \in \M} \C(R, M_1 S) \times \C(M_1 S, M_2 S) \times \C(M_2 S, R).\]
  This implies that
  \begin{align*}
    \rep{ l;\phi_S;(Ml')}{ (Mr'); c ;(Ml')}{(Mr') ; r}
    &= \rep{ l;(Ml');\phi_{NA}}{ (Mr'); c ;(Ml')}{(Mr') ; r} && \text{(functoriality)} \\
    &= \rep{ l;(Ml')}{ \phi_{NA};(Mr'); c ;(Ml')}{(Mr') ; r} && \text{(coend relation)} \\
    &= \rep{ l;(Ml')}{ (Mr');\phi_{S}; c ;(Ml')}{(Mr') ; r} && \text{(functoriality)}
  \end{align*}
  And similarly for the other generating relation, $\rep{l}{ c;\phi_S}{r } = \rep{l}{c}{\phi_S; r }$.

  Therefore, the chain of relations proving $\rep{l}{ r;l}{r } = \rep{l}{ \id_{MA}}{ r }$ can be replicated to show
  \begin{align*}
    \rep{l;(Ml')}{ (Mr');r;l;(Ml')}{(Mr') ; r } &= \rep{l;(Ml')}{ (Mr');\id_{MA};(Ml')}{ (Mr') ; r },
  \end{align*}
  which is equal to $\rep{l;(Ml')}{ M(r'l')}{ (Mr') ; r }$.

  Similarly, a generating relation $\rep{l';\psi_A}{ c'}{ r' } = \rep{l'}{ \psi_A ; c'}{ r' }$ in \[\int^{N_1, N_2 \in \M} \C(S, N_1 A) \times \C(N_1 A, N_2 A) \times \C(M_2 A, S)\]
  implies that
  \begin{align*}
    \rep{l;M(l';\psi_A)}{ Mc'}{ (Mr') ; r }
    &= \rep{l;Ml';M\psi_A}{ Mc'}{ (Mr') ; r } \\
    &= \rep{l;Ml'}{ M\psi_A ; Mc'}{ (Mr') ; r } \\
    &= \rep{l;Ml'}{ M(\psi_A;c')}{ (Mr') ; r }
  \end{align*}
  So again we can replicate the chain of relations proving $\rep{l'}{ r';l'}{ r' } = \rep{l'}{ \id_{MA}}{ r' }$ to show that
  \[\rep{l;(Ml')}{ M(r'l')}{ (Mr') ; r } = \rep{l;(Ml')}{ \id_{MNA}}{ (Mr') ; r } \]
...
\end{proposition}
\begin{proof}
  If $p = \rep{l}{ r }$ is lawful, \todo{todo}
\end{proof}
...
\begin{proof}
  ...
  \begin{align*}
    \twice(\rep{l}{ r })
    &= \rep{l}{ r;l}{ r } \\
    &= \rep{l}{ \phi A}{ r } && \text{($r;l = \phi A$)}\\
    &= \rep{l (\phi A)}{ \id_{MA}}{ r } && \text{(coend relation)} \\
    &= \rep{l;r;l}{ \id_{MA}}{ r } && \text{($r;l = \phi A$ again)}\\
    &= \rep{l}{ \id_{MA}}{ r }  && \text{($l;r = \id_S$)}\\
    &= \once(\rep{l}{ r })
  \end{align*}
\end{proof}
...
\begin{proof}
  ...
  Similarly two representatives $\rep{l}{ r } : S \hto A$ and $\rep{l'}{ r' } : S \hto A$ with complements $M$ and $N$ respectively are identified when there exists a $\phi : M \to N$ such that
  \[
    \begin{tikzcd}
  ...
  Now suppose $p = \rep{l}{ r } : S \hto A$ is a lawful optic, so $rl = \id_S$ and $\langle lr \rangle = \langle \psi A\rangle$. There therefore must exist a finite chain of relations $lr = u_1 \leftrightsquigarrow \dots \leftrightsquigarrow u_n = \psi A$.
  ...
  \[
    ...
    \end{tikzcd}
  \]
  Define $l' = \phi_A l$ and $r' = rk$. The above diagram is a witness that $\rep{l}{ r } = \rep{l'}{ r' }$ as an optic $S \hto A$.
  ...
  A symmetric argument shows that if instead the relation faces leftward, so $lr \leftsquigarrow g$, there exists $l'$ and $r'$ so that $\rep{l}{ r } = \rep{l'}{ r' }$ and $l'r' = u_2^2$.

  We can now inductively apply the above argument to the shorter chain \[l'r' = u_1^2 \leftrightsquigarrow \dots \leftrightsquigarrow u_n^2 \leftrightsquigarrow (\psi A)^2 = \psi^2 A,\] obtained by squaring each morphism in the original chain, until we are left with a representative $\rep{l^*}{ r^* }$ such that $l^*r^* = \psi^N A$, for some $N>0$. This pair $\rep{l^*}{ r^* }$ is the required representative.
\end{proof}
...
\begin{proposition}
  ...
  \end{align*}
  that sends an object $D \in \D$ and optic $\rep{l}{ r } : F \hto G$ in $\Optic_{[\D, \C]}$ to the optic $\rep{l_D}{ r_D } : FD \hto GD$ in $\Optic_\C$.
\end{proposition}
\begin{proof}
  \todo{todo}
\end{proof}
