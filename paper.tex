...
\begin{theorem}
$\Optic_\C$ is symmetric monoidal, where $(S, S') \otimes (T, T') = (S \otimes T, T' \otimes S')$, the unit is $(I, I)$, and the action on morphisms is given by:
\begin{center}
...
\begin{proposition}
$\iota : \C \times \C^\op \to \Optic_\C$ is a strong monoidal functor. \qed
\end{proposition}
...
\end{itemize}
\end{definition}
Note that because $\C_d$ is symmetric monoidal and has the same collection of objects as $\C$, the symmetric monoidal structure of $\C$ must consist of dualisable maps.
...
\item The counit $\varepsilon_{(S, S')}$  is given by the image of the symmetry $\id_{S \otimes S'}$ under the isomorphism of Proposition \ref{prop-costates}. 
\end{itemize}
\end{proposition}
\begin{proof}
...
\todo{TODO: the dual ordering has since been flipped:}
\begin{center}
...
\begin{theorem}
Suppose $\C$ is a symmetric monoidal category and $\T$ is a teleological category. For all monoidal functors $F : \C \to \T_d$, there exists a teleological functor $K : \Lens_\C \to \T$, unique up to isomorphism, such that $K\iota \cong F$.
...
Haskell's optics \todo{find an older reference from DB theory?} allow \emph{polymorphic updates}, where the type of the codomain of the lens can be changed, causing a corresponding change in the domain. As an example, the lens into the first entry of a tuple has type \mbox{\texttt{first :: Lens (a, b) (a', b) a a'}}, then:
\begin{center}
\texttt{set first (1, 5) "hello" == ("hello", 5)}
\end{center}
Note that the type has been changed from $\texttt{(Int, Int)}$ to $\texttt{(String, Int)}$

Polymorphic optics can be captured as follows. For any monoidal category $\C$, and any category $\D$, there is an action of $\C$ on the functor category $[\D, \C]$ which an object $C \in \C$ pointwise with a functor $F \in [\D, \C]$:
\begin{align*}
\C \times [\D, \C] &\to [\D, \C] \\
C, F &\mapsto  C \otimes F(-)
\end{align*}
