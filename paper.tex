...
Just as lenses are given by two concrete maps $\fget : S \to A$ and $\fput : S \times A \to S$, prisms are constructed by two maps $\freview : A \to S$ and $\fmatching : S \to S \sqcup A$. These names are taken from the Haskell \texttt{lens} library.
\begin{align*}
...
\end{align*}
So, if we are given a pre-prism $\langle l, r \rangle : (S, S) \hto (A, A)$ then associated $\freview$ and $\fmatching$ morphisms are given by $\freview = r \inr$ and $\fmatching = [r\inl, \id_S]l$
...
\todo{The following original attempt is wrong, does not capture the right thing}
...
This suggests the following definition of merely well-behaved optics, which we may as well call semi-optics.
