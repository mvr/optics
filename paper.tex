...
\todo{Contributions:
  ...
  \item A definition of lawful optic that specialises in the correct way and allows us to easily derive ad-hoc laws for new kids of optic
...
To capture the \texttt{Iso}s, \texttt{Traversal}s and \texttt{Setter}s of the Haskell \lenslib{} library (see~\ref{lawful-examples}), we generalise to the case of a monoidal action of one category on another.
...
\begin{proof}
  ...
  Now let $\rep{l}{r}$ be a representative for $p$, so $rl = \id_S$ and $\langle lr \rangle = \langle \psi A\rangle$. There therefore exists a finite chain of relations $lr = u_1 \leftrightsquigarrow \dots \leftrightsquigarrow u_n = \psi A$.
  
Suppose the first relation faces rightward, so there exists a $k$ and $\phi$ with $r;l = \phi_A;k$ and $u_2 = k;\phi_A$. Define $l' = l;\phi_A$ and $r' = k;r$. Then:
\begin{align*}
\rep{l'}{r'} 
&= \rep{l ; \phi_A}{k ; r} \\
&= \rep{l}{\phi_A ; k ; r} \\
...
The general pattern is as follows. Once we choose a particular monoidal action $\M \to [\C, \C]$, we find an isomorphism between the set of optics $(S, S') \hto (A, A')$ and a set $\mathbb{O}((S, S'), (A, A'))$ that is easier to describe. Ideally, we then also find a simplified description $\mathbb{D}(S, A)$ for the set \[ \int^{M_1, M_2 \in \M} \C(S, M_1 A) \times \C(M_1 A, M_2 A) \times \C(M_2 A, S)\] appearing in the definition of a lawful optic. We can then ``read off'' what conditions on an element of $\mathbb{O}((S, S), (A, A))$ are needed to ensure that the corresponding element of $\Optic_\M(S, A)$ is lawful. With no offence intended, we will call these more specific conditions the `ad-hoc' laws.
...
If we ask more of our category $\C$, we can upgrade this result to show that a lens $S \hto A$ implies the existence of a complement $\C$ with $S \cong C \times A$. This is a generalised from the argument given in~\cite[Corollary 13]{AlgebrasAndUpdateStrategies} for $\Set$.
...
\end{align*}
where  $\circ$ denotes internal composition and $\nameof{\id_A} : 1 \to \homC(A, A)$ is the `name of' $\id_A$, and similarly for $\id_S$.
\end{proposition}
\begin{proof}
...
A folklore variant of a prism is the \emph{affine traversal}, which allows access to a target that may or may not be present. These are described in~\cite{AffineTraversalPost}, and it is suggested they correspond to pairs of maps $\C(S, A \sqcup S') \times \C(S\times A', S')$. This is not precisely correct, but very close:
...
This set includes into $\C(S, A \sqcup S') \times \C(S\times A', S')$, but the two are not equal.
