...
In this section we work in the case $\C = \Set$. \todo{define applicative/traversable functors}
...
Finally, we encode $\Fold$s, which in $\Set$ are functions $S \to \mathrm{List}\,A$. The key is that the $\mathrm{List}$ functor is the terminal $\Traversable$ functor, so we have isomorphisms
\begin{align*}
