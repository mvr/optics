...
\begin{itemize}
\item Given $f : A \to B$ and $g : A \to C$, I write $[f, g] : A \to B \times C$.
...
In favourable conditions, the converse holds as we can construct a complement:

\begin{proposition}
\todo{This is a guess:} Suppose $\C$ has finite limits and coequalisers, and that there is a morphism $1 \to A$. Then if $\fget : S \to A$ and $\fput : S \times A \to S$ satisfy the three lens laws then the corresponding $\langle l, r \rangle : S \hto A$ is a lens.
\end{proposition}
\begin{proof}
\todo{would be nice if this were true}
\end{proof}

In particular, this holds in $\Set$ so long as $A$ is not empty. In \cite[Corollary 13]{AlgebrasAndUpdateStrategies} a similar fact is proven for lenses expressed as algebras for a particular monad.
