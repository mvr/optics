...
\title{The Categorical Structure of Optics}
...
\end{align*}
If $l : S \to M \otimes A$ and $r : M \otimes A' \to S'$, we write $\langle l, r \rangle : (S, S') \hto (A, A')$ for the image of this pair in $\Optic_\C((S, S'), (A, A'))$, and say that $M$ is the complement for this representative.

\begin{remark}
Optics will always be written with a crossed arrow $\hto$, to distinguish them from morphisms of $\C$.
\end{remark}

Elements of $\Optic_\C((S, S'), (A, A'))$ have a suggestive interpretation as string diagrams with a ``hole'' missing. We draw the pair $\langle l, r \rangle$ as
\begin{center}
\input{diagrams/generic-optic.tikz}
\end{center}
using the \todo{strange} convention of reading string diagrams from bottom to top.
...
Graphically, this is placing the second optic into the hole of the first:
\begin{center}
\todo{todo}
\end{center}
...
\begin{proposition}
For any $\C$, there is a functor $(-, -)_* : \C \times \C^\op \to \Optic_\C$, which is given on objects by $(S, S')_* = (S, S')$ and for a morphism $(f, g) : (S, S') \to (A, A')$ by $(f, g)_* = \langle \lambda_A^{-1} f, g \lambda_{A'} \rangle$.
\end{proposition}
\begin{proof}
...
\begin{theorem}
$\Optic_\C$ is symmetric monoidal, where $(S, S') \otimes (T, T') = (S \otimes T, S' \otimes T')$, the unit is $(I, I)$, and with the action on morphisms induced by
\begin{align*}
...
Note that because $(S, S')_* = (S, S')$, required equations for $(-, -)_*$ to be a monoidal functor hold by definition (although we don't yet know that $\Optic_\otimes$ is monoidal). The pentagon and triangle axioms then hold in $\Optic_\otimes$, as they are the image of the same axioms in $\C \times \C^\op$ under $(-, -)_*$. The only remaining piece to verify is that these structure maps are natural in $\Optic_\otimes$. 
...
\begin{proposition}
These assignments assemble into a \todo{pseudo?}functor $\Optic : \SymmMonCat \to SymmMonCat$.
\end{proposition}
\begin{proof}
\todo{todo}
\end{proof}
...
As noted in \cite{CoherenceForLenses}, 
...
\begin{itemize}
\item A wide symmetric monoidal subcategory $\C_d$ of dualisable morphisms, with an involutive symmetric monoidal functor $(-)^* : \C_d^\op \to \C_d$
\item A family of morphisms $\epsilon_X : X \otimes X^* \to I$, called \emph{counits}, natural with respect to the morphisms in $\C_d$,
\end{itemize}
...
\begin{proposition}
With the above data, the category $\Optic_\C$ is a teleological category.
\end{proposition}
\begin{proof}
\todo{todo}
\end{proof}
...
As conjectured by Jules Hedges, the structure of a teleological category is precisely what is added by $\Optic$.
\begin{theorem}
...
Let $\C$ be a category and $(\M, \otimes, I)$ a monoidal category that acts on $\C$.

\section{Lawful Optics}
In this section we will focus only on preoptics of the form $p : (S,S) \hto (A, A)$. In what follows, we abbreviate $\Optic_\M((S, S), (A, A))$ as $\Optic_\M(S, A)$ and $p : (S, S) \hto (A, A)$ as $p : S \hto A$.
