...
%\todo{Past work:
%  \begin{itemize}
%  \item The Haskell lens people: edwardk, shachaf (Shachaf Ben-Kiki), xplat, roconnor... (TODO: find out who these people are)
...
%\todo{maybe do as string diagram}
%  \begin{align*}
...
\begin{definition}
  Let $\C$ be a category and $(\M, \otimes, I)$ a monoidal category. An \emph{action of $\M$ on $\C$} is a monoidal functor $a : \M \to [\C, \C]$. For two objects $M \in \M$ and $A \in \C$, the action $a(M)(A)$ is abbreviated $MA$.
\end{definition}
...
\begin{proposition}
  ...
  \end{align*}
  that sends an object $D \in \D$ and optic $\rep{l}{r } : F \hto G$ in $\Optic_{[\D, \C]}$ to the optic $\rep{l_D}{r_D } : FD \hto GD$ in $\Optic_\otimes$. This functor preserves lawfulness.
\end{proposition}
\begin{proof}
  \todo{todo}
\end{proof}
...
Many proposed definitions of effectful lenses~\cite{ReflectionsOnMonadicLenses} have modified one or both of $\fget$ and $\fput$ to produce results wrapped in a monadic action. There are disadvantages to this approach: it is not obvious what the laws ought to be and there is no clear generalisation to other optic variants. The general definition of optic given in Section~\ref{sec:optics} suggests we instead work with the Kleisli category $\C_T$ of some effectful monad $T : \C \to \C$.

\todo{define Kleisli category?} For a morphism $f : X \to Y$ in $\C_T$, we write $\underline{f} : X \to TY$ for its underlying morphism in $\C$.

This presents its own set of difficulties. Kleisli categories are only monoidal when the monad in question is \emph{commutative}~\cite{PremonoidalCategories}, which rules out many monads of interest. A premonoidal structure is not sufficient: composition of optics would in that case not be well defined.

But this does not preclude the existence of monoidal actions on $\C_T$. In fact, there is a common monoidal action that has long been used under a different guise:
...
&= \int^{M \in \C} \C_T(S, M \monact A) \times \C(M \times A', T S') \\
&\cong \int^{M \in \C} \C_T(S, M \monact A) \times \C(M, \homC(A', T S')) \\
&\cong \C_T(S, \homC(A', T S') \monact A) \\
&\cong \C(S, T(\homC(A', T S') \times A))
\end{align*}

Concrete effectful lenses therefore consist of a single morphism $\munzip : S \to T(\homC(A', T S') \times A)$.
...
A concrete effectful lens is lawful iff
  \begin{align*}
    ...
    T(\coev_{\homC(A, TS), A} \otimes A)\munzip &= T((T\munzip \circ -) \otimes A)\munzip 
  \end{align*}
\end{proposition}
...
We can say more once we make a specific choice of strong monad.
...
We can find a concrete description of optics for the action $\monact : \C \times \C_{T_Q} \to \C_{T_Q}$:
\begin{align*}
\Optic((S, S'), (A, A'))
...
\begin{remark}
  The right adjoint to $U$ is described in \cite{NotionsOfComputationAsMonoids}, and was used to investigate Haskell's \mintinline{haskell}{Arrow} typeclass.
\end{remark}
