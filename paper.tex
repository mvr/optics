...
There is a common pattern in some of the examples to follow: the evaluation-at-$A$ functor $-A : \M \to \C$ has a right adjoint, say $R_A : \C \to \M$.

In this case we can always eliminate the coend and find a concrete description of an optic:
\begin{align*}
&\int^{F \in \M} \C(S, F\cdot A) \times \C(F\cdot A', S') \\
&\cong \int^{F \in \M} \C(S, F \cdot A) \times \M(F, R_{A'} S') \\
&\cong \C(S,  (R_{A'} S') \cdot A)
\end{align*}

\begin{theorem}\label{thm:optics-are-coalgebras}
An concrete optic $p : S \to  (R_{A} S) \cdot A$ is lawful iff it is a coalgebra for the comonad $X \mapsto (R_{A} X) \cdot A$.
\end{theorem}
\begin{proof}
\todo{todo!}
\end{proof}
...
\begin{remark}
This theorem is a generalisation of \cite[Section 3.2]{RelatingAlgebraicAndCoalgebraic} for lenses.
\end{remark}

%Ross Street:
%More usually - . b has a right adjoint. This is the same as saying that the M-actegory
%C is an M-category (hom enriched in M) which is tensored. So we have
%
%M(m, \hat{C}(b,a)) \cong C(m.b , a)
...
\todo{These are just the coalgebra laws (hopefully)}
...
\begin{proposition}
If $T : \C \to \C$ is a strong monad then $\C$ acts on $\C_T$ by $X \cdot Y := X \otimes Y$.
\end{proposition}
...
\end{align*}
The category $\mathsf{Aff}$ acts on $\C$ by $(P, Q) \cdot A = P \sqcup (Q \times A)$, in fact, $\mathsf{Aff}$ is cooked up exactly as the closure of the actions $- \times A$ and $- \sqcup A$ under composition. A concrete affine traversal is an element of \[\C(S, S' \sqcup (\homC(A', S') \times A)).\]
